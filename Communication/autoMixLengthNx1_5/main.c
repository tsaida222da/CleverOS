...
void receiver(void)   
{
	  ...
	  while(1)
    {
         RecentLength0	=	nonblockRxOS(0, Irx);
			   RecentLength1	=	nonblockRxOS(1, Frx);
		
			   sendByte('@');
			   for (i=0; i < RecentLength0; i++)
				 {
			       sendByte('0' + Irx[i]);
				 }
			   for (i=0; i < RecentLength0; i++)
...
void transmitter1(void)   
{
    ...
    {
				 x = (count % 10);
         startTx = nonblockTxOS(0, Itx, x, 0);
...
void transmitter2(void)   
{
    ...
    {
			   x = (count % 5);
         startTx = nonblockTxOS(1, Ftx, x, 2);
