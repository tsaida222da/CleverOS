                 ...
                 highestPriority =  findLeastBitOS(ReadyTableOS[index]);
	                   ...
	                   highestPriority += base;
						...
						{ 
							  danger = 1;
							 DISABLE_INTERRUPT;	
	              WaitTickOS[highestPriority] = (unsigned long)ISOLATE;
	             ENABLE_INTERRUPT;	
...
{
	  delayTickOS( (unsigned long)ISOLATE );
}
...
void delayUntilEqualOS(int *a, int *b)
{
	  ...
	  char find= 0;
       ...
       if ( (a != NULL) && (b != NULL) )
       {
          ...
          pauseTaskOS( (unsigned long)INFINITE );		 
       }
		...
		{	
		        ...
		        for( i = 0; i < PENDSIZE; i++  )   
            {
				           ...
				           priority =  EventNumberTaskOS[i].priority;	
								   ...
								   if ( (priority != CurrentPriorityOS) && (WaitTickOS[priority] >= (unsigned long)INFINITE) )
							     {  
