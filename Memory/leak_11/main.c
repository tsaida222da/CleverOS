...
{	 
   ...
   {   
		   sendByte('B');
	     pchar1 = getMemoryOS(0, 8, NULL);   // memory pool = 16 bytes
		 	 print32bits((unsigned int) pchar1 );			 
		   if( pchar1 != 0x0 )
			 {
           pchar2 = pchar1;
			 }
			 
			 pchar1 = getMemoryOS(1, 8, NULL);   // memory pool = 16 bytes
		 	 print32bits((unsigned int) pchar1 );	
			 
			 lackNo = lackMemoryNoOS();
			 if ( lackNo >= 0 )
			 {
			     sendByte('#');
			     sendByte('0' + lackNo );
