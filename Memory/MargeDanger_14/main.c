...
{	 
   ...
   {
		 	 sendByte('D');
	     pchar = getMemoryOS(3, 1);
		   print32bits((unsigned int) pchar );
		   p1 = margeAddressOS(pchar);
 	     print32bits((unsigned int) p1 );
		   p2 = dangerAddressOS(pchar, &bytes);
 	     print32bits((unsigned int) p2 );
		 	 sendByte('@');
		 	 sendByte( '0'+ bytes );
