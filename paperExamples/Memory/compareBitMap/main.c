...
{	
    ...
    { 
			 ...
			 if ( five % 5 == 0 )
			 {
			    ...
			    print32bits( five );
			    sendByte('A');
			    (pointer1 == 0x0) ? sendByte('a') : displayBitmapFree();
       }
			 count++;
			 y1  = count % 8 + 1;
		   delayTickOS(y1);
    ...
    while(1)
    {
	     pointer2 = getMemoryOS(0, x, NULL);
			 if ( display )
			 {
				  display = 0;
			    sendByte('B');				 
			    (pointer2 == 0x0) ? sendByte('b') : displayCleverOSFree();
			 }
			 count++;
			 y3  = count % 8 + 1;		
		   delayTickOS(y3);
...
int main(void)  
{
	   ...
	   sendByte('W'); sendByte('A'); sendByte('I'); sendByte('T');
