...
void task0(void)   
{
    ...
    {
		     sendByte('A');
			   pint = pendMailOS(number, NULL, CLEARMAIL, INFINITEOS);        // 2
		//	   pint = pendMailOS(number, NULL, NOTCLEARMAIL, INFINITEOS);			// 1
				 sendByte('a');	
			   if( pint != 0x0 )
				 {
				    sendByte('0' + *pint);
...
void task1(void)   
{
    ...
    {
		     sendByte('B');
			   pint = pendMailOS(number, NULL, NOTCLEARMAIL, INFINITEOS);
				 sendByte('b');
			   if( pint != 0x0 )
				 {
				    sendByte('0' + *pint);
...
void task2(void)   
{
    ...
    {
		     sendByte('C');
			   pint = pendMailOS(number, NULL, CLEARMAIL, INFINITEOS);
				 sendByte('c');
			   if( pint != 0x0 )
				 {
				    sendByte('0' + *pint);
