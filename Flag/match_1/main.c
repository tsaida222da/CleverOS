...
void task0(void)   
{
    ...
    {
		    sendByte('A');
        readAddress = pendMailOS(number, NULL, NOTCLEARMAIL, INFINITE);
			  sendByte('a');
			  for(i=0; i<2;i++)
				{
...
{ 
    ...
    {
		    sendByte('B');
			  pendFlagOS(number, pendFlag1, FLAG_MATCH_ANY, INFINITE );
			  sendByte('b');
...
void task2(void)   
{
    ...
    {
		    sendByte('C');
			  pendFlagOS(number, pendFlag2, FLAG_MATCH_ANY, INFINITE );
			  sendByte('c');
...
void task3(void)  
{
    ...
    {
		    sendByte('D');
			  pendFlagOS(number, pendFlag3, FLAG_MATCH_ALL, INFINITE );
			  sendByte('d');
...
void task4(void)   
{
    ...
    {
		    sendByte('E');
			  pendFlagOS(number, pendFlag4, FLAG_MATCH_ALL, INFINITE );
			  sendByte('e');
...
void task5(void)   
{
   ...
   {
		    sendByte('F');
			  pendFlagOS(number, pendFlag5, FLAG_MATCH_ALL, INFINITE );
		    sendByte('f');
		    writeWord = 0x55555555;
...
void task7(void)  
{
   ...
   {
		    sendByte('H');
			  pendMailOS(number, NULL, NOTCLEARMAIL, INFINITE);
		    sendByte('h');
   }
} 
...
void task8(void)   
{
	 ...
	 while(1)
   {
		    ...
		    writeMail[0]= 0x11111111;
		    writeMail[1]= 0x22222222;
