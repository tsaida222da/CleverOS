...
unsigned int post0 = 0x0f;  // 1111
unsigned int post1 = 0x08;  // 1000
unsigned int post2 = 0x04;  // 0100
...
void task4(void)   
{
    ...
    {
		    sendByte('E');
			  pendFlagOS(number, post1, FLAG_MATCH_ALL, INFINITEOS );
        sendByte('@');
			  for(i=0; i<32; i++)
			  {
					  if( checkPublicFlagBitOS(0, i) )
						{
                sendByte('0' + i);				
						}
				}
