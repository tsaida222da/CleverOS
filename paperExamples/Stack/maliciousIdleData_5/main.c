...
{ 
	 unsigned int  *spAddr;
   extern unsigned int* TaskSpPointerOS[2];
	 int  k = 1;
	 int  number;
	 unsigned int  *malicious;
	
   spAddr = TaskSpPointerOS[0];	 // starting address of stack
	
   while(1) 
   {  
		 	sendByte('A');

			malicious = irregularIdleDataOS(&number);
			
			if ( number > 0 )
			{
          print32bits( *malicious );
			}
			
		  k++;	
		  if ( k == 6 )
			{
				  *(spAddr + 3) = 0xfff;  // inject malicious data
			}
